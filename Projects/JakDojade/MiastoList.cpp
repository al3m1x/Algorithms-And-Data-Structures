...
void MiastoList::LookForNames(char** map, int w, int h) { //funkcja szukaj¹ca nazw i przyporz¹dkowywuj¹ca je do danego miasta
    char buffer[12];
    Miasto* temp = firstNode;
    while (temp != nullptr) { //póki obecne miasto to nie nullptr
        int x = temp->getX();
        int y = temp->getY();
        String name;
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                if (xx < 0 || xx >= w || yy < 0 || yy >= h || (xx == x && yy == y)) {
                    continue; //poza granicami mapy i lokacja gwiazdki
                }
                char c = map[xx][yy];

                if (isalnum(c)) {
                    if (yy + 1 < h || yy - 1 >= 0) { //tylko w granicach mapy
                        if (yy - 1 < 0) { //sytuacje brzegowe
                            int i = 0, j = 1;
                            while ((isalnum(c))) { //zczytujemy znaki dopóki s¹ liter¹ lub cyfr¹
                                buffer[i] = c;
                                c = map[xx][yy + j];
                                i++; j++;
                            }
                        ...
                        else if (yy + 1 >= h) { //sytuacje brzegowe
                            int i = 0, j = 1;
                            while ((isalnum(c))) {
                                buffer[i] = c;
                                c = map[xx][yy - j];
        ...
        if (!name.isEmpty()) {
            temp->setName(name); //zapisanie nazwy miasta w liœcie
        }
        temp = temp->next; //przejœcie do nastêpnego miasta na liœcie
    }
}

void MiastoList::bufferIntoString(int& counter, char buffer[12], char*& buffer2, String& str) { //zamieniamy tablicê charów na wartoœæ typu klasy String
...
void MiastoList::reversedBufferIntoString(int& counter, char buffer[12], char*& buffer2, String& str) { //zamieniamy tablicê charów na wartoœæ typu klasy String, ale na odwrót
    for (int i = 0; i < counter; i++) {
...
Miasto* MiastoList::returnByName(String name) { //iterujemy w poszukiwaniu miasta o danej nazwie
    if (firstNode != nullptr) {
        Miasto* temp = firstNode;
        while (temp != nullptr) {
...
void MiastoList::resetVariables() { //resetujemy wartoœci zmiennych pomocniczych dla ka¿dego miasta w liœcie
    Miasto* temp = firstNode;
    while (temp != nullptr) {
        temp->setVisited(false);
        temp->setDistance(9999999);
